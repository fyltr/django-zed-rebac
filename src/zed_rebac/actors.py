...
_current_actor: ContextVar[SubjectRef | None] = ContextVar("zed_rebac_current_actor", default=None)
_sudo_state: ContextVar[dict[str, Any] | None] = ContextVar("zed_rebac_sudo", default=None)
...
class _ZedSubjectMarker(Protocol):
    """Anything decorated with @zed_subject exposes _zed_type and _zed_id_attr."""

    _zed_type: str
    _zed_id_attr: str


ActorLike = Union[SubjectRef, _ZedSubjectMarker, Any]
...
Concretely accepts:
  ...
  - Any class decorated with `@zed_subject(...)` (→ `<type>:<id_attr_value>`)
"""


_subject_registry: dict[type, tuple[str, str]] = {}
"""Mapping `cls -> (zed_type, id_attr)` populated by `@zed_subject`."""


def zed_subject(*, type: str, id_attr: str = "pk") -> Callable[[type], type]:
...
def to_subject_ref(actor: ActorLike) -> SubjectRef:
    ...
    if isinstance(actor, user_model):
        ...
        return SubjectRef.of(app_settings.ZED_REBAC_USER_TYPE, str(actor.pk))
    ...
    if Group is not None and isinstance(actor, Group):
        return SubjectRef.of(app_settings.ZED_REBAC_GROUP_TYPE, str(actor.pk), "member")

    # @zed_subject-registered?
    ...
    raise NoActorResolvedError(
        ...
        f"Decorate the class with @zed_subject(type=..., id_attr=...) "
...
def grant_subject_ref(agent: Any, on_behalf_of: Any | None) -> SubjectRef:
    ...
    and `agents/grant` subject type via `@zed_subject`. The plugin itself doesn't
    ...
    `ZED_REBAC_ACTOR_RESOLVER` to translate (request, agent, user) into the
    persisted grant id.
    """
...
def sudo(*, reason: str | None = None) -> Iterator[None]:
    ...
    `reason` is mandatory unless `ZED_REBAC_REQUIRE_SUDO_REASON = False`.
    """
    ...
    if app_settings.ZED_REBAC_REQUIRE_SUDO_REASON and not reason:
        raise SudoReasonRequiredError(
            ...
            "ZED_REBAC_REQUIRE_SUDO_REASON is True"
        )
...
def default_resolver(request: Any) -> SubjectRef | None:
    ...
    Override via `ZED_REBAC_ACTOR_RESOLVER = "myapp.path.to.resolver"`.
    """
...
def get_actor_resolver() -> Callable[[Any], SubjectRef | None]:
    ...
    path = app_settings.ZED_REBAC_ACTOR_RESOLVER
