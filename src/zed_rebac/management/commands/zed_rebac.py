"""Single management command with subcommands.
    ...
    python manage.py zed_rebac sync                     # idempotent
    python manage.py zed_rebac sync --check             # CI gate; non-zero on drift
    python manage.py zed_rebac sync --force-overwrite   # destructive
    python manage.py zed_rebac check                    # validate without writes
    python manage.py zed_rebac build-zed                # emit effective.zed
    python manage.py zed_rebac explain <type>.<perm>    # print compiled expression
"""
...
class Command(BaseCommand):
    help = "Manage zed_rebac schema (sync / check / build-zed / explain)."

    def add_arguments(self, parser: Any) -> None:
        sub = parser.add_subparsers(dest="cmd", required=True)

        p_sync = sub.add_parser("sync", help="Load permissions.zed files into Schema* tables")
        p_sync.add_argument("--check", action="store_true", help="CI gate: detect drift; no writes.")
        p_sync.add_argument(
            "--force-overwrite",
            action="store_true",
    ...
    def _handle_sync(self, options: dict[str, Any]) -> None:
        ...
        for app_config in apps.get_app_configs():
            zed_path = self._resolve_zed_path(app_config)
            if zed_path is None:
                continue
            ...
            text = zed_path.read_text(encoding="utf-8")
            ...
            self.stdout.write(f"-> {package_name} ({zed_path})")
        ...
        if check_only:
            if any_drift:
                raise CommandError("Schema drift detected. Run `zed_rebac sync` to apply.")
    ...
    def _resolve_zed_path(self, app_config: Any) -> Path | None:
        # Two ways to declare: `AppConfig.zed_schema = "permissions.zed"` (rel
        # path), or a `permissions.zed` adjacent to apps.py.
        rel = getattr(app_config, "zed_schema", None)
        if rel is not None:
    ...
    def _sync_row(
        ...
        record = (
            PackageManagedRecord.objects.filter(package=package, external_id=external_id).first()
        )
    ...
    def _handle_check(self) -> None:
        any_errors = False
        for app_config in apps.get_app_configs():
            path = self._resolve_zed_path(app_config)
            if path is None:
                continue
    ...
    def _handle_build_zed(self, options: dict[str, Any]) -> None:
        ...
        if out_path is None:
            base_dir = conf_app_settings.ZED_REBAC_SCHEMA_DIR or Path.cwd() / "zed-rebac"
        ...
        chunks: list[str] = ["// Generated by django-zed-rebac. Do not edit.\n", "use typechecking\n"]
    ...
    def _handle_explain(self, options: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise CommandError(f"No permission {perm!r} on {rt}: {exc}")
